...
test('maps protocol v2 Plus and keeps the weekly AI trial available', async ({ page }) => {
  ...
  await expect(comparison.getByText('Save from other browsers')).toBeVisible()
...
test('offers free library setup and explains Pro collection in Settings', async ({ page }) => {
  await openDashboard(page, {
    initialSessions: sessions,
    nativeProtocolVersion: '2',
    entitlementTier: 'free'
...
test('shows Safari connection checks and accepts a bridge arriving after the timeout', async ({ page }) => {
  ...
  await expect(guide).toContainText('Authorize Tab Space')
  await expect(guide).toContainText('Click the Tab Space button in the Safari toolbar.')
  await expect(guide).toContainText('Choose “Always Allow on Every Website” so saving works on every page.')
  // The permission popover mirrors Safari's own, blue default included.
  await expect(guide).toContainText('Website Access')
  await expect(guide).toContainText('Allow for One Day')
  await expect(guide).toContainText('Always Allow on This Website')
  await expect(guide).toContainText('Privacy note')
  // The note carries the app's own auditability links.
  await expect(guide.getByRole('link', { name: 'permission rationale' }))
    .toHaveAttribute('href', 'https://github.com/yuanzhoucq/Tab-Space/issues/15')
  await expect(guide.getByRole('link', { name: 'source repository' }))
    .toHaveAttribute('href', 'https://github.com/yuanzhoucq/Tab-Space')
  await expect(page.getByTestId('pairing-guide')).toHaveCount(0)
...
test('directs visitors without the app to the Tab Space website', async ({ page }) => {
  ...
  await expect(page.getByTestId('pairing-guide').locator('.guide-notes')).toHaveCount(0)
...
test('ignores retired gift eligibility and never claims a trial', async ({ page }) => {
  await openDashboard(page, {
    initialSessions: sessions,
...
test('non-Pro companion library stays usable after a Pro-only refusal', async ({ page }) => {
  await openWebExtensionDashboard(page, ['sessions.read', 'sessions.write', 'ai.v1', 'dashboard.ai.v1', 'subscription.v1'])
  await expect(page.locator('.session')).toHaveCount(2)
  const card = page.getByTestId('session-session-research')
  await card.hover()
  await card.getByTestId('edit-session').click()
  await card.getByTestId('session-title-input').fill('Updated research')
  await card.getByTestId('session-title-input').press('Control+Enter')
  await expect.poll(() => lastBridgeCommand(page, 'UpdateSession')).toMatchObject({
    payload: { bookmarks: [{ title: 'Updated research' }] }
  })
  await page.evaluate(() => window.postMessage({
    channel: 'tabspace-webextension-v2', source: 'extension',
    type: 'request-error', error: { code: 'pro_required' }
  }, window.location.origin))
  await expect(page.getByTestId('plan-comparison')).toBeVisible()
  await page.locator('.subscription-modal .close-btn').click()
  await expect(page.locator('.session')).toHaveCount(2)
  await expect(page.getByTestId('trial-banner')).toHaveCount(0)
})
